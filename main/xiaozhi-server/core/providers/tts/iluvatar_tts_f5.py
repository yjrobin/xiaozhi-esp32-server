...
class TTSProvider(TTSProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        self.url = config.get("url", "https://ai-cloud.4paradigm.com:9443/preview/ttsvc/synthesize")
    ...
    async def text_to_speak(self, text, output_file):
        json_data = {
            "text": text,
            "audio_key": self.voice
            # "fast_infer": 可选，推理速度等级，当前仅支持[0, 1, 2]，默认为0。等级越高，速度越快，但可能质量偏低
        }
        if self.first:
            json_data["fast_infer"] = 1
            self.first = False

        if self.method.upper() == "POST":
            resp = requests.post(self.url, json=json_data, headers=self.headers, auth=self.auth)
        else:
            resp = requests.get(self.url, json=json_data, headers=self.headers, auth=self.auth)
        if resp.status_code == 200:
            ...
            error_msg = f"iluvatar_tts_f5请求失败: {resp.status_code} - {resp.text}"
